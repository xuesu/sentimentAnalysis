...
class Predictor:
    def __init__(self, docs=None, model_path=None, trainable=True):
        ...
        mes_holder.MODEL_SAVE_PATH += self.name
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("Input") as scope:
                ...
                self.train_labels = tf.placeholder(tf.int32, shape=[None, mes_holder.DG_SENTENCE_SZ, mes_holder.LABEL_NUM],
                                                   name="Label")
            # variable
            ...
            with tf.name_scope("Convnet") as scope:
                ...
                for conv_knum, conv_stride, pool_size, pool_stride in zip(mes_holder.PRE_CONVS_L1_KERNEL_NUM,
                    ...
                    for conv2_knum, conv2_stride, pool2_size, pool2_stride in zip(mes_holder.PRE_CONVS_L2_KERNEL_NUM,
                                                                                  mes_holder.PRE_CONVS_L2_STRIDE,
                                                                                  mes_holder.PRE_POOLS_L2_SIZE,
                                                                                  mes_holder.PRE_POOLS_L2_STRIDE):
                        conv2 = tf.layers.conv1d(pool, mes_holder.PRE_CONV_L2_FILTER_NUM, conv2_knum, conv2_stride,
                                                 use_bias=True, activation=tf.nn.relu, padding="same")
            ...
            with tf.name_scope("LSTM") as scope:
                self.lstm = tf.contrib.rnn.BasicLSTMCell(mes_holder.PRE_LSTM_SZ)
            ...
            with tf.name_scope("Linear") as scope:
                self.linear2 = tf.layers.dense(self.lstm_output, mes_holder.PRE_LINEAR2_SZ)
            with tf.name_scope("Output") as scope:
                self.logits = tf.reshape(tf.layers.dense(self.linear2, mes_holder.PRE_LINEAR3_SZ, name="Logits"),
                                         [-1, mes_holder.DG_SENTENCE_SZ, mes_holder.LABEL_NUM])
            ...
            if trainable:
                ...
                if model_path is None:
                    model_path = mes_holder.MODEL_SAVE_PATH + "/model"
                self.session = tf.Session(graph=self.graph)
                self.saver.restore(self.session, model_path)

            self.writer = tf.summary.FileWriter(mes_holder.MODEL_SAVE_PATH + '/logs/', self.session.graph)
    ...
    def train(self, model_path=None):
        ...
        valid_accuracys = []
        try:
            os.mkdir(mes_holder.MODEL_SAVE_PATH)
        except OSError as e:
            print e
        if model_path is not None:
            self.saver.restore(self.session, model_path)
        ...
        for i in range(mes_holder.PRE_STEP_NUM):
            ...
            if i % mes_holder.PRE_VALID_TIME == 0:
                ...
                if accuracy >= self.best_accuracy_valid:
                    ...
                    if test_accuracy >= mes_holder.PRE_GOOD_RATE and average_train_accuracy >= mes_holder.PRE_GOOD_RATE:
                        ...
                        self.saver.save(self.session, mes_holder.MODEL_SAVE_PATH + "/model")
                average_train_accuracy = 0.0
                average_loss = 0.0
        accuracy = self.test(self.session)
        fname = mes_holder.MODEL_SAVE_PATH + "/accuracy.json"
        with open(fname, "w") as fout:
            json.dump([train_accuracys, valid_accuracys], fout)
        fname = mes_holder.MODEL_SAVE_PATH + "/result.txt"
        with open(fname, "w") as fout:
