...
class DataGenerator:
    ...
    def words2vec(self, words, ind):
        ans = []
        words_sz = len(words)
        for wi in range(ind, ind + self.sentence_sz):
            if wi < words_sz and words[wi][2] is not None:
                ans.append(self.words_id.get(words[wi][2], 0))
            else:
                ans.append(0)
        return ans

    def label2vec(self, label, ind):
        ans = [[0] * Mes.LABEL_NUM for _ in range(self.sentence_sz)]
        words_sz = len(label)
        for wi in range(ind, ind + self.sentence_sz):
            if wi < words_sz:
                ans[wi - ind][label[wi] + 1] = 1
            if wi >= words_sz:
                break
        return ans
...
if __name__ == '__main__':
    hotel = pymongo.MongoClient("localhost", 27017).paper['semval14_laptop']
